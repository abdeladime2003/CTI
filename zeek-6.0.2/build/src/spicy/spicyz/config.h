...
inline const auto InstallBinDir() { return path("/usr/local/zeek") / "bin"; }

inline const auto LibraryPath() { return get_env_path_or("ZEEK_SPICY_LIBRARY_PATH", "/usr/local/zeek/share/zeek/spicy"); }

inline const auto ModulePath() { return get_env_path_or("ZEEK_SPICY_MODULE_PATH", "/usr/local/zeek/lib/zeek/spicy"); }

inline const auto DataPath() { return get_env_path_or("ZEEK_SPICY_DATA_PATH", "/usr/local/zeek/share/zeek"); }

inline const auto CxxZeekIncludesDirectories() {
    std::string includes;
    add_path(includes, path("/usr/local/zeek") / "include");
...
#define SPICY_VERSION_NUMBER 10802
#define ZEEK_VERSION_NUMBER 60002

inline const auto SpicyVersion = "";
inline const auto ZeekVersion = "6.0.2";
inline const auto InstallPrefix = path("/usr/local/zeek");
